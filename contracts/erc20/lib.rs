...
mod erc20 {
    ...
    impl Erc20 {
        ...
        /// Get `totalSupply` from an ERC20 contract.
        #[ink(message)]
        pub fn balance_of(&self, to: [u8; 20]) -> u128 {
            // const SELECTOR: [u8; 4] = hex!["70a08231"];

            let encoded = Self::balance_of_encode(to.into());
        ...
        /// Send `mint` call to ERC20 contract.
        #[ink(message)]
        pub fn mint(&mut self, to: [u8; 20], value: u128) -> bool {
            let encoded_input = Self::mint_encode(to.into(), value.into());
            self.env()
                .extension()
                .xvm_call(
                    super::EVM_ID,
        ...
        fn balance_of_encode(to: H160) -> Vec<u8> {
            let mut encoded = BALANCE_OF_SELECTOR.to_vec();
            let input = [Token::Address(to)];
            encoded.extend(&ethabi::encode(&input));
            encoded
        }

        fn mint_encode(to: H160, value: U256) -> Vec<u8> {
